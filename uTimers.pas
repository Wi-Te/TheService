  ...
  try
    ...
    if Result > 0 then begin
      ...
      for i := Result - 1 downto 0 do begin
        ...
        raise Exception.Create('Неверный формат: '+inp);
      end;
...
begin
  for i := Length(Timers) - 1 downto 0 do begin
    ...
    if timers[i].sched = 0 then begin
      if (timers[i].start <= 0) then s := s + #13#10 + 'Не указано время старта [start]';
      if (timers[i].untyl <= 0) then s := s + #13#10 + 'Не указано время окончания [until]';
      if (timers[i].every <= 0) then s := s + #13#10 + 'Не указан интервал [every]';
      if (timers[i].every > 12*60) then s := s + #13#10 + 'Интервал [every] больше 12ч';
    end else begin
      if (timers[i].start <> 0) or (timers[i].untyl <> 0) or (timers[i].every <> 0) then
        s := s + 'Указано расписание [sched]. Интервал и время начала/окончания здесь лишние';
    end;
    if s <> '' then
      raise Exception.Create('Некорректно сформулирован таймер ['+timers[i].naim+']:'+s);
  end;
end;
...
begin
  ...
  if t.start > t.untyl then begin //16.10 - 7.00
    if n < t.untyl then begin //утром до отсечки
      ...
      else tnext := t.start; //вышли за таймер, целимся на вечер
    end else if n < t.start then tnext := t.start //уже поздно и еще рано одновременно. Целимся на вечер
      ...
      else tnext := t.start + 24*60; //целимся на вечер завтра
    end;
  end else begin //7.30 - 16.10
    if n < t.start then tnext := t.start //мы рано, целимся на начало
    else if n >= t.untyl then tnext := t.start + 24*60 //мы поздно, целимся на завтра
      ...
      if tnext < t.untyl then ex := False //остались в рамках таймера
      else tnext := t.start + 24*60; //целимся на завтра
    end;
  end;
...
begin
  ...
  //n - в минутах, как и times[], как и всё остальное

  {нет нужды запоминать последнее срабатывание таймера и проверять на совпадение с теперешним моментом
  таймер заводится на начало первой секунды той минуты, что он должен сработать. Плю миллисекунды, которые тут игнорируются
  плюс время на обработку события таймера и рестарт таймера. В любом случае сюда, мы попадаем не раньше. чем в ту же минуту
  а этот расчет никогда не целится на эту самую минуту. Только на следующую и дальше. Так что никаких нулевых и отрицательных интервалов}
